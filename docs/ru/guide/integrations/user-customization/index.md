...
Скорость анимации `PatternMap` и яркость `EmissionMap` регулируются в [скрипте камуфляжа](#camouflages) следующими параметрами:
...
1. Paint.NET не обладает обширным количеством функций, как в Photoshop, что ухудшит наши возможности по созданию текстур. Но хорошим отличием в нашей ситуации является предустановленное сохранение изображения в формате `.dds`.
2. В дальнейших разделах будет использование `плагинов` которые облегчают пользование приложением. Скачать плагины Вы можете [здесь]().
   > :::details Установка плагинов
   >
   > 1. Скачиваем `.dll` и переносим её в папку `Effects`, находящуюся в корневой папке скачанного нами Paint.NET.
   >
   > ![Папка установки Paint.NET](./assets/Paint.NET_save_plagins.png)
   >
   > 2. Теперь запускаем или перезапускаем (если он был запущен) Paint.NET и проверяем наличие плагинов во вкладке `Эффекты`.
   >
   > :::

3. Заходим в `Paint.NET` и создаём файл с размерами, имеющими соотношение сторон `1:1`.

:::details Paint.NET - Сохранение текстур

Сохранение получившихся текстур в папку `camouflages` производится по алгоритму:

1. `Файл` -> `Сохранить как...` -> `DirectDraw Surface (DDS) (*.dds)`.

   ![Сохранение файла](./assets/Paint.NET_save_texture.png)
   
2. В параметрах сохранения выбираем `Равномерную` оптимизацию сжатия.
3. Активируем `Генерацию MIP-текстур` (выбор метода сжатия MIP-текстур на Ваше усмотрение).

   ![Параметры сохранения](./assets/Paint.NET_save_texture(params).png)

::: warning Важно!
Также нужно понимать что в игре используются текстуры с методами кодирования изображения - `DXT1`, `DXT3`, `DXT5`. Лучше выбрать `DXT5`, так как будет корректно отображаться `Альфа канал`(прозрачность).

:::

   ![Тип кодированя изображения](./assets/Paint.NET_save_texture(DXT).png)
...
1. Создаём основу узора. 

   ![Создаём основу узора](./assets/Paint.NET_AM_texture.png)

2. Заходим в контекстное меню, выбираем `Эффекты` -> `Искажение` -> `Seamless Texture Maker`.
3. В этом плагине есть функция создания бесшовной текстуры путём отзеркаливания изображения.

   ![Готовая функция SeamlessTextureMaker](./assets/Paint.NET_AM_texture_example.png)

   > ![]
   > Данный плагин может либо разрезаеть, либо дублировать изображение, вставить и повернуть его в таком порядке чтобы исключить его резкий обрыв.
   > Данный вариант может зациклить любое изображение, но больше подходит для "простого" вида, состоящего из простейших элементов(линий, градиентов, фигур), так как на более сложных(рисунках, фотографиях) будет виден явный зеркальный стык.
4. Так же в этом плагине есть `помошник` создания бесшовных текстур.

   ![Помошник создания бесшовной текстуры](./assets/Paint.NET_AM_texture_helper.png)

   > ![]
   > Он "разрезает" изображение и вставляет в таком порядке чтобы можно было самостоятельно редактировать стык.
5. Если необходимо, смягчаем переход стыка.

   ![Пример сглаживания стыка](./assets/Paint.NET_AM_texture_helper_example.png)

6. Сохраняем получившийся файл по написанному ранее алгоритму.

:::

#### PatternMap — текстура {#PatternMap}

Создание `PatternMap` состоит из:
1. Изначального `"черного"`(#000000) состояния текстуры - отсутствие свечения.
   > ![]
   > `PatternMap` принимает только `красный канал` (#`00`0000).
   > Но для удобства его создают черно-белым.
2. Создания на текстуре `коэффицента` свечения (процентное соотношение от `EmissionBrightness`). Пример, если значение красного канала = 127 => яркость свечения = 50% от максимального значения.
   > ![]
   > 1. `коэффицент` принимает не всё изображение, а только среднее значение строк `PatternMap` (не знаю всех или отдельных) => рисовать узоры на `PatternMap` бесмысленно, достаточно создания градиента направленного горизонтально.

   ![Пример градиента](./assets/Paint.NET_pattern_texture.png)

   > 2. длина строки = такту обновления за период времени прописанный в `emissionAnimationSpeed`
   > (Например, изображение 8x8 писелей и Speed=1с => каждые 0,125с будет изменятся яркость свечения)

#### EmissionMap — текстура {#EmissionMap}

Создание `EmissionMap`(RGBA) состоит из:
1. создания `основного цвета`(RGB) - определяет цвет свечения. 
   > ![]
   > К тому же, чем ближе цвет к черному, тем менее яркое свечение => черным цветом будет характирезоваться отсутствие какого-либо свечения (при `любых` значениях `PatternMap` и `Alpha-Channel`), что очень помогает при создании анимации, занимающей не всю площадь текстуры.
2. создания `карты прозрачности`(Alpha-Channel) - определяет очерёдность свечения.
   > ![]
   > 1. Анимация будет идти: от наибольшего значения к наименьшему => от непрозрачного к прозрачному => от белого к черному (на `карте прозрачности`).
   > 2. `Alpha-Channel` принимает значения от `0` до `255`.
   > 3. Но при значениях равных `0` или `255` анимация свечения будет отсутствовать => свечение будет бесконечным и будет соответствовать максимальному коэффиценту свечения из `EmissionBrightness`.
...
:::details Paint.NET
  ...
  > ![] Для более удобного пользования в Paint.NET нам нужен плагин – Alpha Mask.

1. Создаём `1-ую` текстуру (RGB) и рисуем на ней «основу» стиля.

   ![Цветовая основа анимированной текстуры](./assets/Paint.NET_emission_texture(RGB).png)

2. Создаём `2-ую` текстуру желательно черно-белую и сохраняем в формате `.png` (таким образом будет легче понять какова будет прозрачность выходного пикселя).

   ![Альфа канал анимированной текстуры](./assets/Paint.NET_emission_texture(Alpha).png)

   > Эта текстура будет картой прозрачности (Alpha-канал).
3. Переходим к `1-ой` текстуре, заходим в контекстное меню, выбираем `Эффекты` -> `Alpha Mask`. В открывшемся окне нажимаем на `Browse...` и выбираем нашу `2-ую` текстуру в формате `.png`.
   
   ![Применение AlphaMask](./assets/Paint.NET_emission_texture_AlphaMask.gif)
   
4. На выходе получаем полупрозрачную текстуру у которой будет коректно отображаться анимация.

   ![Итоговая анимированная текстура](./assets/Paint.NET_emission_texture(RGBA).png)

   ![Пример аанимаации на танке](./assets/Paint.NET_emission_texture_result.gif)
...
Локализация строк для стиля хранится в файлах формата `.mo`, которые делаются из файлов формата `.po`, которые имеют следующий вид:
...
Скрипты элементов стиля хранятся в формате `.xml` и полностью идентичны по строению внутриигровым файлам в папке `scripts/item_defs/customization/<mod_id>_<element_type>.xml`, где
...
::: warning Внимание!
Далее описываются секции для типа элемента `camouflage`, которых в одной `itemGroup` может находиться несколько. Точные ограничения не регламентированы.
:::
...
Для эффекта металла и бликов используйте либо:
...
::: warning Внимание!
Далее описываются секции для типа элемента `decal`, которых в одной `itemGroup` может находиться несколько. Точные ограничения не регламентированы.
:::

* `root.itemGroup.decal.id` — уникальный числовой идентификатор.
* `root.itemGroup.decal.texture` — путь до текстуры.
* `root.itemGroup.decal.mirror` — булевое значение (истина, ложь), означающее необходимость отразить по горизонтали декаль.
...
::: warning Внимание!
Далее описываются секции для типа элемента `attachment`, которых в одной `itemGroup` может находиться несколько. Точные ограничения не регламентированы.
:::
...
::: warning Внимание!
Далее описываются секции для типа элемента `insignia`, которых в одной `itemGroup` может находиться несколько. Точные ограничения не регламентированы.
:::
...
где `begin` и `end` — начальные и конечные координаты используемой области относительно левого верхнего угла изображения в процентах (1.0 = 100%).
:::
...
::: warning Внимание!
Далее описываются секции для типа элемента `modification`, которых в одной `itemGroup` может находиться несколько. Точные ограничения не регламентированы.
:::
...
::: warning Внимание!
Далее описываются секции для типа элемента `paint`, которых в одной `itemGroup` может находиться несколько. Точные ограничения не регламентированы.
:::
...
::: tip Совет
Секция `root.itemGroup.paint.userString`, хоть и не будет отображаться в клиенте, может быть использована для удобства ориентирования в коде.
:::
...
::: warning Внимание!
Далее описываются секции для типа элемента `personal_number`, которых в одной `itemGroup` может находиться несколько. Точные ограничения не регламентированы.
:::
...
::: warning Внимание!
В этом типе элемента нет секции `itemGroup`. В этом файле также могут быть описаны несколько шрифтов.
:::
...
::: warning Внимание!
Далее описываются секции для типа элемента `projection_decal`, которых в одной `itemGroup` может находиться несколько. Точные ограничения не регламентированы.
:::
...
* `root.itemGroup.projection_decal.mirror` — булевое значение (истина, ложь), означающее возможность отразить по горизонтали и по вертикали проекционную декаль.
...
::: warning Внимание!
Далее описываются секции для типа элемента `style`, которых в одной `itemGroup` может находиться несколько. Точные ограничения не регламентированы.
:::
...
* `alternateItems.<element_type>` — перечень альтернативных элементов, где `element_type` — тип элемента так, как вы его называли в XML-файлах. Например, не `paints`, а `paint`. Не `personalnumbers`, а `personal_number`.
...
::: warning Внимание!
Далее описываются секции для субэлемента `outfit`, которых в `outfits` может находиться несколько. Точные ограничения не регламентированы.
:::
