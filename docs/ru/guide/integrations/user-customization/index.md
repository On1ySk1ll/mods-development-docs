...
2. создания `карты прозрачности`(Alpha-Channel) - определяет очерёдность свечения в соответствии с `PatternMap`.
   > ![]
   > 1. `Alpha-Channel` принимает значения от `0` до `255`.
   > 2. Но при значениях равных `0` или `255` анимация свечения будет отсутствовать => свечение будет бесконечным и будет соответствовать максимальному коэффиценту свечения из `EmissionBrightness`.
